class Day_4b
  require 'set'
  def run(input)
    numberOfCorrectPhrases = 0
    passphrases = input.split("\n")
    for passphrase in passphrases
        words = passphrase.split(" ")
        uniqueWords = words.to_set
        if words.count == uniqueWords.count && !containPalindromes(uniqueWords)
          numberOfCorrectPhrases += 1
        end
    end
    numberOfCorrectPhrases
  end

  def containPalindromes(uniqueWords)
    compareWords = []
    for word in uniqueWords
      compareWords << word.chars.sort.join
    end
    p uniqueWords
    p compareWords
    if uniqueWords.count == compareWords.to_set.count
      return false
    end
    return true
  end
end
print Day_4b.new.run("kvvfl kvvfl olud wjqsqa olud frc
